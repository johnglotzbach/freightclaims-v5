...
export const claimsController = {
  ...
  getById: asyncHandler(async (req: Request, res: Response) => {
    const user = getUser(req);
    const claim = await claimsService.getById(req.params.id, user);
    res.json(claim);
  }),
  ...
  delete: asyncHandler(async (req: Request, res: Response) => {
    const user = getUser(req);
    await claimsService.delete(req.params.id, user);
    res.status(204).send();
  }),
  ...
  updateStatus: asyncHandler(async (req: Request, res: Response) => {
    const user = getUser(req);
    const claim = await claimsService.updateStatus(req.params.id, req.body.status, user);
    res.json(claim);
  }),

  // --- Parties ---
  getParties: asyncHandler(async (req: Request, res: Response) => {
    const parties = await claimsService.getParties(req.params.id);
    res.json(parties);
  }),

  addParty: asyncHandler(async (req: Request, res: Response) => {
    const party = await claimsService.addParty(req.params.id, req.body);
    res.status(201).json(party);
  }),

  updateParty: asyncHandler(async (req: Request, res: Response) => {
    const party = await claimsService.updateParty(req.params.id, req.params.partyId, req.body);
    res.json(party);
  }),

  removeParty: asyncHandler(async (req: Request, res: Response) => {
    await claimsService.removeParty(req.params.id, req.params.partyId);
    res.status(204).send();
  }),

  // --- Products ---
  getProducts: asyncHandler(async (req: Request, res: Response) => {
    const products = await claimsService.getProducts(req.params.id);
    res.json(products);
  }),

  addProduct: asyncHandler(async (req: Request, res: Response) => {
    const product = await claimsService.addProduct(req.params.id, req.body);
    res.status(201).json(product);
  }),

  updateProduct: asyncHandler(async (req: Request, res: Response) => {
    const product = await claimsService.updateProduct(req.params.id, req.params.productId, req.body);
    res.json(product);
  }),

  removeProduct: asyncHandler(async (req: Request, res: Response) => {
    await claimsService.removeProduct(req.params.id, req.params.productId);
    res.status(204).send();
  }),

  // --- Comments ---
  getComments: asyncHandler(async (req: Request, res: Response) => {
    const comments = await claimsService.getComments(req.params.id);
    res.json(comments);
  }),

  addComment: asyncHandler(async (req: Request, res: Response) => {
    const user = getUser(req);
    const comment = await claimsService.addComment(req.params.id, req.body, user);
    res.status(201).json(comment);
  }),

  // --- Tasks ---
  getTasks: asyncHandler(async (req: Request, res: Response) => {
    const tasks = await claimsService.getTasks(req.params.id);
    res.json(tasks);
  }),

  addTask: asyncHandler(async (req: Request, res: Response) => {
    const user = getUser(req);
    const task = await claimsService.addTask(req.params.id, req.body, user);
    res.status(201).json(task);
  }),

  updateTask: asyncHandler(async (req: Request, res: Response) => {
    const task = await claimsService.updateTask(req.params.id, req.params.taskId, req.body);
    res.json(task);
  }),

  deleteTask: asyncHandler(async (req: Request, res: Response) => {
    await claimsService.deleteTask(req.params.id, req.params.taskId);
    res.status(204).send();
  }),

  // --- Payments ---
  getPayments: asyncHandler(async (req: Request, res: Response) => {
    const payments = await claimsService.getPayments(req.params.id);
    res.json(payments);
  }),

  addPayment: asyncHandler(async (req: Request, res: Response) => {
    const payment = await claimsService.addPayment(req.params.id, req.body);
    res.status(201).json(payment);
  }),

  updatePayment: asyncHandler(async (req: Request, res: Response) => {
    const payment = await claimsService.updatePayment(req.params.id, req.params.paymentId, req.body);
    res.json(payment);
  }),

  // --- Identifiers ---
  getIdentifiers: asyncHandler(async (req: Request, res: Response) => {
    const identifiers = await claimsService.getIdentifiers(req.params.id);
    res.json(identifiers);
  }),

  addIdentifier: asyncHandler(async (req: Request, res: Response) => {
    const identifier = await claimsService.addIdentifier(req.params.id, req.body);
  ...
  getAcknowledgement: asyncHandler(async (req: Request, res: Response) => {
    const ack = await claimsService.getAcknowledgement(req.params.id);
    res.json(ack);
  }),

  createAcknowledgement: asyncHandler(async (req: Request, res: Response) => {
    const user = getUser(req);
    const ack = await claimsService.createAcknowledgement(req.params.id, req.body, user);
